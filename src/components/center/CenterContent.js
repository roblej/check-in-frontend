...
import { HelpCircle, MessageCircle, Plus, ChevronDown, AlertTriangle } from 'lucide-react';
...
const CenterContent = ({ initialFaqs, initialInquiries, initialSearchParams }) => {
  ...
  // 신고 카테고리
  const reportCategories = ['전체', '부정확한 정보 제공', '서비스 불만', '청결 문제', '시설 문제', '기타'];
  ...
  useEffect(() => {
    ...
    } else if (activeTab === '문의') {
      fetchInquiries().finally(() => setLoading(false));
    } else if (activeTab === '신고') {
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2">
              <input
                type="text"
                placeholder={activeTab === 'FAQ' ? '질문을 검색하세요...' : activeTab === '문의' ? '문의 제목을 검색하세요...' : '신고 제목을 검색하세요...'}
                value={searchTerm}
            ...
            <div>
              <select
                ...
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
                {(activeTab === 'FAQ' ? faqCategories : activeTab === '문의' ? inquiryCategories : reportCategories).map(category => (
          ...
          <div className="flex justify-between items-center">
            <div className="text-sm text-gray-600">
              {activeSearchTerm || categoryFilter !== '전체' ? (
                ...
                <span>전체 {activeTab === 'FAQ' ? 'FAQ' : activeTab === '문의' ? '문의' : '신고'}를 표시하고 있습니다.</span>
              )}
            </div>
                  ...
                  <div key={report.id} className="p-6">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                            report.status === '완료' ? 'bg-blue-100 text-blue-800' :
                        ...
                        {report.contentId && (
                          <button
                            onClick={() => router.push(`/hotel/${report.contentId}`)}
                        ...
                        {report.status === '완료' && (
                          ...
                          >
                            ...
                            />
                            {expandedAnswer === report.id ? '답변 숨기기' : '답변 보기'}
                          </button>
                        )}
                      </div>
                    </div>
                    ...
                    {expandedAnswer === report.id && (
                      <div 
                        className="overflow-hidden transition-all duration-500 ease-in-out mt-4"
                        style={{
                          maxHeight: expandedAnswer === report.id ? '500px' : '0px',
                          opacity: expandedAnswer === report.id ? 1 : 0,
                          transform: expandedAnswer === report.id ? 'translateY(0)' : 'translateY(-10px)'
                        }}
                      >
                        <div className="p-4 bg-blue-50 rounded-lg border border-blue-200">
                          {answers[report.id] ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-sm text-red-700">호텔 관련 문제를 신고하세요</p>
            </div>
          </div>
        </div>
      </div>
